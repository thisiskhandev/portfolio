...
const Portfolio = (props) => {
  ...
  const randomList = shuffle(CardsPortfolioArr);
    ...
    <>
      <main className="container cardss portfolioMain" id="portfolioMain">
        ...
        <section className="row mt-5 g-4">
          {randomList.slice(limit).map((val, index) => {
            return (
              <div className={BootClasses[0].CardClass} key={index}>
        ...
        <CTA name="View All" target="/projects" />
      </main>
    </>
  );
};
